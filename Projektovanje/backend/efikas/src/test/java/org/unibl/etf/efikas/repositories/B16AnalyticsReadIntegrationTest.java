...
class B16AnalyticsReadIntegrationTest {
    ...
    void calculatesFinancialOccupancyTaskAndWorkforceMetricsFromBusinessFacts() {
        ...
        reservations.changeStatus(cancelled.reservationId(),
                new ChangeReservationStatusRequest(
                        org.unibl.etf.efikas.models.enums.ReservationStatus.CANCELLED, "Cancelled test stay"),
                agent.getEmail());
    ...
    private AppUser worker(String label, org.unibl.etf.efikas.models.entities.Specialization specialization) {
    ...
    private static long count(java.util.List<org.unibl.etf.efikas.models.responses.AnalyticsResponse.NamedCount> rows,
                              String name) {
        return rows.stream().filter(row -> row.name().equals(name)).mapToLong(
                org.unibl.etf.efikas.models.responses.AnalyticsResponse.NamedCount::count).findFirst().orElse(0);
    }
