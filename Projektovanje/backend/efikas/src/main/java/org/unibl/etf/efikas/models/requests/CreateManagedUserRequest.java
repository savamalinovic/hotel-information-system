...
@Schema(description = "Manager-created user account. Password is write-only and never returned.")
public record CreateManagedUserRequest(
        @NotBlank @Email @Size(max = 50) String email,
        @NotBlank @Size(min = 8, max = 128) @Schema(accessMode = Schema.AccessMode.WRITE_ONLY) String password,
        ...
        Set<@Positive Short> specializationIds
) {
    public CreateManagedUserRequest {
        specializationIds = specializationIds == null ? Set.of() : Set.copyOf(specializationIds);
    }
}
