package org.unibl.etf.blueStars.openapi;
...
import org.unibl.etf.blueStars.controllers.AppUserController;
import org.unibl.etf.blueStars.controllers.AuthController;
import org.unibl.etf.blueStars.controllers.HotelProfileController;
import org.unibl.etf.blueStars.controllers.LeaveRequestController;
import org.unibl.etf.blueStars.controllers.NotificationsController;
import org.unibl.etf.blueStars.controllers.SpecializationController;
import org.unibl.etf.blueStars.controllers.TaskController;
import org.unibl.etf.blueStars.controllers.UserManagementController;
import org.unibl.etf.blueStars.controllers.WorkforceController;
import org.unibl.etf.blueStars.security.JwtUtil;
import org.unibl.etf.blueStars.services.AppUserService;
import org.unibl.etf.blueStars.services.HotelProfileService;
import org.unibl.etf.blueStars.services.SpecializationService;
import org.unibl.etf.blueStars.services.StoreService;
import org.unibl.etf.blueStars.services.UserManagementService;
...
class OpenApiContractTest {
    ...
    void generatedV1ContractContainsVersionedTypedLoginAndStableErrors() throws Exception {
        mockMvc.perform(get("/v3/api-docs/v1"))
                ...
                .andExpect(jsonPath("$.info.title").value("BlueStars Hotel Information System API"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/notifications/push-token'].post").exists())
                .andExpect(jsonPath("$.paths['/api/v1/notifications/push-token/unregister'].post").exists())
                .andExpect(jsonPath("$.paths['/api/v1/notifications/push-token/unregister'].post.responses['204']").exists())
                .andExpect(jsonPath("$.paths['/api/v1/notifications/push-token/unregister'].post.requestBody"
                        + ".content['application/json'].schema.$ref")
                        .value("#/components/schemas/UnregisterPushNotificationTokenRequest"))
                .andExpect(jsonPath("$.components.schemas.UnregisterPushNotificationTokenRequest.properties.token.type")
                        .value("string"))
                .andExpect(jsonPath("$.components.schemas.UnregisterPushNotificationTokenRequest.properties.userId")
                        .doesNotExist())
                ...
                .andExpect(jsonPath("$.components.schemas.CreateLeaveRequest.properties.startDate.format")
                        .value("date"))
                .andExpect(jsonPath("$.components.schemas.CreateLeaveRequest.properties.endDate.format")
                        .value("date"))
                .andExpect(jsonPath("$.components.schemas.LeaveRequestResponse.properties.startDate.format")
                        .value("date"))
                .andExpect(jsonPath("$.components.schemas.LeaveRequestResponse.properties.endsAt").doesNotExist())
