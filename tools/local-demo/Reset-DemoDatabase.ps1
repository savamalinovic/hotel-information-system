...
try {
    ...
    try {
        ...
        $exists = & $psql @baseArgs '-tAc' "SELECT 1 FROM pg_database WHERE datname = '$DatabaseName'"
        ...
        if ($exists) {
            ...
            & $psql @baseArgs '-c' "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$DatabaseName' AND pid <> pg_backend_pid();"
            if ($LASTEXITCODE -ne 0) { throw 'Could not close existing demo database connections.' }
            & $psql @baseArgs '-c' "DROP DATABASE `"$DatabaseName`";"
        ...
        & $psql @baseArgs '-c' "CREATE DATABASE `"$DatabaseName`";"
